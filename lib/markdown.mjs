...
function inline(text, ctx = {}) {
  ...
  out = out.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_, label, href) => {
    ...
    const peptrivaShop = external && /(?:^|\/\/|\.)peptriva\.com(?:[:/?#]|$)/i.test(href);
    const rel = external ? (peptrivaShop ? ' rel="noopener sponsored"' : ' rel="noopener"') : '';
...
/** Strip inline markup from a single run of text. */
...
/**
 * Strip markdown to plain prose. Used for meta descriptions and for the parts of
 * a page that carry no component blocks.
 *
 * This deliberately drops ::: blocks. Use renderPlain for anything a reader or an
 * answer engine will consume, or the component payloads vanish.
 */
...
/**
 * Render markdown to plain text WITH the component blocks expanded.
 *
 * The markdown mirrors and llms-full.txt are the surfaces answer engines read, so
 * a FAQ page whose answers live inside a ::: block has to carry those answers
 * here too. ctx.plainComponents maps a block name to (props, body) => string.
 */
export function renderPlain(src, ctx = {}) {
  const lines = String(src).replace(/\r\n/g, '\n').split('\n');
  const out = [];
  let i = 0;
  ...
  while (i < lines.length) {
    ...
    if (comp) {
      ...
      i += 1;
      const fn = ctx.plainComponents?.[name];
      const rendered = fn ? fn(props, bodyLines.join('\n')) : '';
      if (rendered) out.push(rendered.trim(), '');
      continue;
    }

    if (/^>\s?/.test(line)) {
      out.push(plainInline(line.replace(/^>\s?/, '')));
      i += 1;
      continue;
    }
