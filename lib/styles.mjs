...
.cover .container.cover-split { grid-template-columns: minmax(0, 1.12fr) minmax(0, 0.88fr); }
.cover .container + .container { margin-top: clamp(1.4rem, 3vw, 2.2rem); }
...
/* Hero plate: a diagram of what the page argues, drawn from its own numbers. */
.cover-figure { margin: 0; }
.cover-figure svg { width: 100%; height: auto; display: block; border: 1px solid var(--rule); background: var(--paper); }
.cover-figure figcaption { font-size: .82rem; color: var(--ink-faint); margin-top: .55rem; max-width: 68ch; }
.cover-figure figcaption:empty { display: none; }

/* Per-page KPI strip, carried on every page like the reference network. */
.kpi-wrap .stat-strip { margin: 0; }
.kpi .stat dt { font-size: .64rem; }
.kpi .stat dd { font-size: 1.08rem; }

/* Masthead issue line. */
.issue-strip { border-top: 1px solid var(--rule-soft); background: var(--card); }
.issue-strip p {
  margin: 0; padding: .42rem 0; font-size: .68rem; font-weight: 600;
  letter-spacing: .14em; text-transform: uppercase; color: var(--ink-faint);
}

/* Second navigation row, for the site whose chapters do not fit one line. */
.nav .nav-row-2 { margin-top: .2rem; }
.nav .nav-row-2 a { font-weight: 400; color: var(--ink-soft); }
...
.badge-google { background: rgba(66,133,244,0.1); color: #1a73e8; border: 1px solid rgba(66,133,244,0.35); }
...
.cta-band {
  ...
  padding: 1.5rem 1.6rem; margin: 2.6rem 0 0; max-width: var(--measure);
}
.cta-band-title { font-family: var(--font-display); font-weight: 700; font-size: 1.16rem; margin-bottom: .5rem; color: var(--ink-strong); }
.cta-band p:last-of-type { margin-bottom: 0; }
.cta-band .cta-row { margin-top: 1.1rem; }
...
.layout-main > hr { max-width: var(--measure); margin-inline: auto; }
.layout-main .related { max-width: var(--measure); margin-inline: auto; }
.ad-rail {
  position: sticky;
  top: 5.2rem;
  display: grid;
...
.ad-link:hover { outline: 2px solid var(--accent); outline-offset: 1px; }

/* Keep prose measure rules working inside .layout-main */
.prose .layout-main > section,
.prose .layout-main > p,
.prose .layout-main > ul,
.prose .layout-main > ol,
.prose .layout-main > blockquote,
.prose .layout-main > hr { max-width: var(--measure); margin-inline: auto; }

.prose .layout-main .stat-strip,
.prose .layout-main .index-cards,
.prose .layout-main .table-scroll {
  --breakout: min(calc(var(--container) - (2 * var(--gutter)) - 16rem), calc(100vw - (2 * var(--gutter)) - 16rem));
  width: var(--breakout);
  max-width: none;
  margin-inline: calc((100% - var(--breakout)) / 2);
}

.vendor-shop { margin: 0.55rem 0 0; font-size: 0.9rem; }
.vendor-shop a { font-weight: 600; }

/* Editorial vendor ranking */
.vendor-rank { list-style: none; margin: 1.6rem 0; padding: 0; max-width: var(--measure); counter-reset: none; }
.vendor-row { border-top: 1px solid var(--rule); padding: 1.1rem 0 1.25rem; }
.vendor-row:last-child { border-bottom: 1px solid var(--rule); }
.vendor-featured { background: color-mix(in srgb, var(--accent) 7%, transparent); margin: 0 -0.85rem; padding-left: 0.85rem; padding-right: 0.85rem; border-radius: 4px; }
.vendor-head { display: flex; align-items: baseline; gap: 0.75rem; flex-wrap: wrap; }
.vendor-rank { font-family: var(--font-display); font-weight: 800; color: var(--ink-faint); min-width: 2.2rem; }
.vendor-name { font-family: var(--font-display); font-weight: 700; font-size: 1.05rem; flex: 1; }
.vendor-badge { display: inline-block; margin-left: 0.55rem; font-family: var(--font-body); font-size: 0.72rem; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; color: var(--accent); vertical-align: middle; }
.vendor-score { font-family: var(--font-display); font-weight: 800; font-size: 1.3rem; color: var(--ink-strong); }
.vendor-score .of { font-size: 0.62em; color: var(--ink-faint); font-weight: 600; }
.vendor-band { margin: 0.45rem 0 0; font-size: 0.86rem; font-weight: 600; color: var(--accent); }
.vendor-blurb { margin: 0.35rem 0 0; font-size: 0.92rem; color: var(--ink-soft); }
.vendor-row .meter { margin-top: 0.55rem; }

/* In-issue and sister-site related links */
.related { margin: 2.4rem 0 0.6rem; padding-top: 1.6rem; border-top: 1px solid var(--rule); }
.related h2 { font-size: var(--h3); margin: 0 0 0.8rem; }
.related ul { list-style: none; padding: 0; margin: 0; display: grid; grid-template-columns: repeat(auto-fit, minmax(14rem, 1fr)); gap: 0.45rem 1.2rem; }
.related li { margin: 0; }
.related a { font-weight: 600; }
.related-note { display: block; color: var(--ink-faint); font-size: var(--small); font-weight: 400; }
.related-network { border-top-style: dashed; }
...
@media (max-width: 560px) {
  .ad-rail { grid-template-columns: 1fr; max-width: 20rem; }
}
...
@media print {
  .masthead, .site-footer, .cta-row, .nav-toggle, .ad-rail { display: none; }
  body { background: #fff; }
